...
Podríamos considerar una función $f: RR^n -> RR$ monosemántica en el contexto de una representación $h: X -> RR^n$, si la composición $f circle h$ depende principalmente de una única propiedad intepretable del espacio de entrada $X$
...
Podríamos considerar una función $f: RR^n -> RR$ polisemántica respecto a una representación $h: X -> RR^n$, si la composición $f circle h$ no dependte de múltiples propiedades distintas del espacio de entradas $X$, sin que una sola de ellas domine claramente sobre las demás.
